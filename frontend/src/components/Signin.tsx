...
const Signin: React.FC = () => {
    ...
    const [teacherCred, setTeacherCred] = useState({
        email: "",
        password: "",
        teacherCode: "" // example extra field for teachers
    })
    ...
    function handleStudentSubmit(e: FormEvent) {
        e.preventDefault()
        try {
            login({ ...studentCred })
        } catch {
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
            ...
            <div className="w-full max-w-md bg-white shadow-lg rounded-xl p-6">
                ...
                <div className="flex items-center justify-center mb-6">
                    <div className="relative w-48 h-10 bg-gray-200 rounded-full flex items-center cursor-pointer">
                        <div
                            className={`absolute top-0 bottom-0 w-1/2 bg-indigo-700 rounded-full transition-transform duration-300 ${role === "teacher" ? "translate-x-full" : "translate-x-0"}`}
                        ></div>
                        <div className="relative z-10 flex w-full text-sm font-medium">
                            <button
                                type="button"
                                className={`flex-1 py-2 rounded-full transition-colors duration-300 ${role === "student" ? "text-white" : "text-gray-600"}`}
                            ...
                            <button
                                type="button"
                                className={`flex-1 py-2 rounded-full transition-colors duration-300 ${role === "teacher" ? "text-white" : "text-gray-600"}`}
                    ...
                    <form onSubmit={handleStudentSubmit} className="space-y-5">
                        <div>
                            ...
                            <input
                                ...
                                className="w-full p-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                value={studentCred.email}
                                onChange={(e) => setStudentCred({ ...studentCred, email: e.target.value })}
                            />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Password</label>
                    ...
                    <form onSubmit={handleTeacherSubmit} className="space-y-5">
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => setTeacherCred({ ...teacherCred, email: e.target.value })}
                            />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Password</label>
                            <input
                                ...
                                onChange={(e) => setTeacherCred({ ...teacherCred, password: e.target.value })}
                            />
                        </div>
                        <div>
                            ...
                            <input
                                ...
                                className="w-full p-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                value={teacherCred.teacherCode}
                                onChange={(e) => setTeacherCred({ ...teacherCred, teacherCode: e.target.value })}
                            />
                        </div>
