...
const Notes: React.FC<{ account: string }> = ({ account }) => {
    ...
    const fetch_content = async () => {
        ...
        const folderId = params.pathname.split('/').at(-2)

        try {
            fetch('http://localhost:3000/api/v1/student/content/' + `${folderId}` + '/' + `${subFolderId}`, {
                method: 'GET',
                headers: {
    ...
    const addNewContent = (e: FormEvent) => {
        ...
        try {
            ...
            fetch('http://localhost:3000/api/v1/teacher/content/' + `${subjectId}` + '/' + `${subFolderId}`, {
                method: 'POST',
                body: formData
